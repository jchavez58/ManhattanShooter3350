...
public:
	X11_wrapper() {
		...
		setupScreenRes(1200, 480); //640,480
...
void createRaindrop(const int n)
{
	...
	for (i=0; i<n; i++) {
		...
		//larger line/width = faster speed
...
//Sprite1 ();
  ...
  float wid = 120.0f; //120
	...
	if (g.forest) {
		glBindTexture(GL_TEXTURE_2D, g.forestTexture);
		glBegin(GL_QUADS);
			glTexCoord2f(0.0f, 1.0f); glVertex2i(0, 0); //(0,0)
			glTexCoord2f(0.0f, 0.0f); glVertex2i(0,g.yres); //(0,g.yres)
			glTexCoord2f(1.0f, 0.0f); glVertex2i(g.xres, g.yres); //(g.xres, g.yres);
  ...
  if (g.showBigfoot) {
    ...
    glTranslatef(bigfoot.pos[0], bigfoot.pos[1], bigfoot.pos[2]);
		...
		glEnd();
		glPopMatrix();
		// couch beign rendered *move to personal file*
     //Sprite1();
    if (g.trees && g.silhouette) {
      ...
      glBegin(GL_QUADS);
       glTranslatef(65,65,0.0);
        ...
        glTexCoord2f(1.0f, 0.0f); glVertex2i(200, 200); //(g.xres, g.yres);
